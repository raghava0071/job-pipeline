...
def print_header():
    if not HAS_RICH:
        print("\n" + "="*60)
        print("  JOB PIPELINE — Your Name")
        print("="*60 + "\n")
        return
    console.print()
    console.print(Panel.fit(
        "[bold cyan]🚀  JOB PIPELINE — Your Name[/bold cyan]\n"
...
def step_cover_letters(df):
    ...
    name      = full_profile.get("name", "Your Name")
    summary   = ce.build_profile_summary(full_profile)
    title_col = next((c for c in df.columns if "title"   in c.lower()), df.columns[0])
    co_col    = next((c for c in df.columns if any(k in c.lower()
                     for k in ["company", "employer", "organization"])), None)
...
# ── MAIN ──────────────────────────────────────────────────────────────────────
def main():
    p = argparse.ArgumentParser(description="Job Pipeline — Your Name")
