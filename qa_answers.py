...
try:
    ...
    _NAME      = PROFILE.get("name", "Your Name")
    _FIRST     = _NAME.split()[0]
    _LAST      = _NAME.split()[-1]
    _EMAIL     = PROFILE.get("email", "your.email@example.com")
    _PHONE     = PROFILE.get("phone", "5555555555").replace("(","").replace(")","").replace(" ","").replace("-","")
    _LOCATION  = PROFILE.get("location", "City, ST")
    _CITY      = _LOCATION.split(",")[0].strip()
    _STATE     = _LOCATION.split(",")[1].strip() if "," in _LOCATION else "FL"
    _LINKEDIN  = PROFILE.get("linkedin", "linkedin.com/in/yourusername")
    ...
    _CITY      = "City"
    _STATE     = "FL"
    _LINKEDIN  = "linkedin.com/in/yourusername"
...
QA = {
    ...
    # ── True/False radio variants for authorization ───────────────────────────
    # Some Indeed/LinkedIn forms use True/False radio buttons instead of Yes/No
    ...
    "are you currently authorized to work in the us for":
        "True",
    ...
    # ── True/False radio variants for sponsorship ─────────────────────────────
    "do you require any kind of visa sponsorship now or in the future":
        "False",

    "require any kind of visa sponsorship":
        "False",
    ...
    # ── Commuting ─────────────────────────────────────────────────────────────
    # NOTE: These were causing an infinite loop — pipeline filled "Yes" via cache
    # but the radio button never registered, so Vision kept retrying "No" forever.
    # Fix: answer "Yes" (we're open to commuting for hybrid/on-site roles we chose
    # to apply to). The radio JS click path now uses .check() not .fill(), so it
    ...
    # ── EEOC — Disability DATE field ─────────────────────────────────────────
    # Federal contractors show a "Voluntary Self-Identification of Disability Date"
    # field that requires today's date in MM/DD/YYYY format.
    # The pipeline was getting stuck here 22+ times — now handled with today's date.
    "voluntary self-identification of disability date":
        _date.today().strftime("%m/%d/%Y"),

    "self-identification of disability date":
        _date.today().strftime("%m/%d/%Y"),

    "disability identification date":
        _date.today().strftime("%m/%d/%Y"),

    "date of self-identification":
        _date.today().strftime("%m/%d/%Y"),
    ...
    "what was your work email":     "your.email@example.com",
    "previous work email":          "your.email@example.com",
    "your work email":              "your.email@example.com",
    "email address used":           "your.email@example.com",
    ...
    # State: store full name "Florida" — it matches both dropdown values AND
    # text inputs. The select handler in linkedin_apply_now.py also tries "FL"
    # as a fallback, so either format works regardless of the form's expectation.
